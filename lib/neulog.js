...
class NeuLogGsr extends EventEmitter
{
		...
		return co(function*()
		{
			...
			const connectionAttempts = results.map((portData) =>
			{
				return new Promise((resolve, reject) =>
				{
					...
					logger.once('connect', () => resolve(logger));
					logger.connect();
				});
			});
	...
	connect()
	{
		this.serial.open((err) =>
		{
			...
			const timeoutHandle = setTimeout(() => this.onError(new GsrConnectFailedError(this.port)), 2000);
			this.once('connect', () => clearTimeout(timeoutHandle));
			this.once('error', () => clearTimeout(timeoutHandle));
